...
impl BspRequest for BuildTargetCompileRequest {
  const METHOD: &'static str = "buildTarget/compile";

  type Result = BuildTargetCompileResult;
}

#[derive(Debug, Default, Serialize, Deserialize, Clone, PartialEq)]
#[serde(rename_all = "camelCase")]
pub struct BuildTargetCompileRequest {
...
pub struct BuildTargetCompileResult {
  ...
  /// Kind of data to expect in the `data` field. If this field is not set, the
  /// kind of data is not specified.
